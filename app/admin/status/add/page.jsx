...
import { collection, addDoc, getDocs, where, query, serverTimestamp } from "firebase/firestore";
...
const AddPage = () => {
	...
	const submitHandler = async (e, downloadURL) => {
		...
		try {
			...
			toast.success(`Status Added with ID: ${docRef.id}`);
			handleReset();
			setIsLoading(false);
		} catch (error) {
	...
	const handleUpload = async (e) => {
		...
		const q = query(collection(db, "status"), where("statusName", "==", statusName));
		try {
			...
			if (matchWithExisting.length > 0) {
				toast.error(`Status with name ${statusName} already exists!`);
				setIsLoading(false);
			} else {
				...
				<Card as={"form"} onSubmit={(e) => { handleUpload(e) }} className="w-full dark:bg-zinc-800 dark:text-white mx-auto has-[[aria-label=Loading]]:!pointer-events-none [&_label]:has-[[aria-label=Loading]]:!pointer-events-none">
					<CardHeader className="flex gap-3">
						...
						<div className="flex flex-col">
							<p className="text-lg font-bold text-primary dark:text-white">Add Status</p>
